...
To identify the Xpath, we will need to install a Chrome extension named [SelectorGadget](http://selectorgadget.com/) which will help know the Xpath and the CSS selector for the specific element we want to extract.

Once we've installed Selector Gadget, go to the Wikipedia page and try to select the entire table we want to extact, here the opinion polls for the first round. 
![](./img/select.gif)
As you can see above, you will need to move around with the cursor to select the entire table and once it's selected, click to get the Xpath. After you click, it will be available at the bottom if you press `Xpath`. You should get the following Xpath : `//*[contains(concat( " ", @class, " " ), concat( " ", "wikitable", " " ))]`

Then you will need to tell `rvest` your discovery by reporting the Xpath in the `html_node` fonction which extracts specific elements defined by their Xpath. The `html_table` fonction will import that data into a dataframe named `polls_round1`. 
