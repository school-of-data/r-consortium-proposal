...
couple of useful commands in R. Let's now first work with `elic_2016_q1_raw`:
...
It looks like the variable `Typ` needs a bit of cleaning:
```{r}
...
Let's sort the country in the plot by number of exports to it. `summarize()` helps
us computing the number of exports, `order()` helps us getting the correct order,
and `factor()` helps us define that we want the country levels according to a
fixed order. `ggplot()` automatically understands that it should plot the factor
levels in the order given.
...
And now one, which shows how much worth the goods were that 
were exported to each country. Again we want to have that in a specific order.
...
p <- ggplot(elic_2016_4, aes(x = Wert)) + stat_ecdf()
p

p + coord_cartesian(xlim = c(0, 10^5))
...
levels(elic_2016_4$Land2)[!(levels(elic_2016_4$Land2) %in% highest10)] <- "andere"
...
The cool thing with `ggplot2` for a newsroom is that you can have different 
themes.
...
p_cntry + theme_classic(base_family = "Open Sans") + 
  theme(axis.text = element_text(family = "Open Sans Bold", colour = scodagrey),  
