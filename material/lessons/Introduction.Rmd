...
mapdat <- gbr_sp@data
ni <- mapdat[mapdat$NAME_1 == "Northern Ireland",]

gbr <- fortify(gbr_sp, region = "HASC_2")
gbr$id[gbr$id %in% ni$HASC_2] <- "Northern Ireland"
gbr_brexit <- merge(gbr, datahasc, by.x = "id", by.y = "HASC_code", all.x = TRUE)
ord <- order(gbr_brexit$order)
gbr_brexit <- gbr_brexit[ord, ]
# nim <- gbr_brexit[gbr_brexit$id %in% ni$HASC_2, ]
# datahasc[datahasc$HASC_code == "Northern Ireland", ]
