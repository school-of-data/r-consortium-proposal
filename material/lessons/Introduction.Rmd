...
library("foreign") 
```
If you use RStudio, you can also install and load the packages through Install button in the Packages tab and checkboxes in the package list in the same tab. Note: If you want to install all packages for the whole lesson, repeat the same thing for `dplyr`, `gmodels`, `ggplot2`, `GGally`, `raster`, `maptools`, `broom`, and `gpclib` packages or run the following commands:
```{r, eval=FALSE}
